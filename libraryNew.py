...
def delete1(MEMBER='',REFERENCE='',FIRSTNAME='', SURNAME='' , ADDRESS='',PINCODE='',MOBILE='',BOOKID='', TITLE='', AUTHOR='', BORROWED='',DATEDUE='',loandays='', RETURNFINE=''):
    ...
    c.execute('delete from Student3 where FIRSTNAME=:firstname',{'firstname':e3.get()})
    # record=c.fetchall()
    #
    #
    # for i in record:
    #     booklist.delete(END,i)
...
def search1():
    ...
    book=tk.messagebox.askokcancel('SEARCH OPTION','YOU CAN ONLY SEARCH FORM BOOKID,MOBILENO or FIRSTNAME')
    ...
    c.execute('SELECT * FROM Student3 WHERE BOOKID=:bookid OR FIRSTNAME=:firstname OR MOBILE=:mobile ' ,{'bookid':e8.get() ,'firstname':e3.get(),'mobile':e7.get()})
    row=c.fetchall()
